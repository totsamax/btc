//taxi.controller('OfferDetails', ['$scope', '$localStorage', '$pusher', 'offerService', '$http', '$state', '$ionicModal', '$ionicPlatform', '$ionicPopup', '$stateParams', '$cordovaLocalNotification', '$cordovaVibration', function ($scope, $localStorage, $pusher, offerService, $http, $state, $ionicModal, $ionicPlatform, $ionicPopup, $stateParams, $cordovaLocalNotification, $cordovaVibration) {
...
//        $scope.offer = offerService.currentOffer;
//        $scope.orderId = offerService.orderId;
...
//            $scope.modal = modal;
//
//            if ($stateParams.continueOrderId && $stateParams.continueOrderId !== '') {
...
//                    $scope.status = "Машина выехала, ожидайте, пожалуйста";
//                    $scope.modal.show();
//                    offerService.AssignToDriver($localStorage.lastOrderId, $scope.offer.DriverId).$promise.then(
//                            function (resp) {
//                                if (!channel) {
//                                    var channel = pusher.subscribe('order_id' + $scope.orderId);
//                                } else {
//                                    channel = pusher.subscribe('order_id' + $scope.orderId);
//                                }
//                                ;
//
...
//            offerService.CancelByUser($scope.orderId);
//            $scope.modal.hide();
//            $state.go('home');
//        };
//
                            ...
                            function (resp) {
                                ...
                                switch (resp.OrderState) {
                                    case 0:
                                    {
                                        ...
                                        $http({
                                            method: 'GET',
                                            url: 'http://bishkektaxi-api-prod.azurewebsites.net/GetDriverOfferList',
                                            params: {
